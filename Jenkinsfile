pipeline {
  ...
  stages {
    ...
    stage('Checkout Code') {
      steps {
        script {
          def repoUrl = 'https://github.com/arthurhzna/user-service_ci-cd'
    ...
    stage('Commit and Push Changes') {
      steps {
        script {
          ...
          git remote set-url origin https://${GITHUB_CREDENTIALS_USR}:${GITHUB_CREDENTIALS_PSW}@github.com/arthurhzna/user_service_cicd.git
        ...
        withCredentials([
          ...
          script {
            ...
            ssh -o StrictHostKeyChecking=no -i ${SSH_KEY} ${USERNAME}@${HOST} '
              ...
              sed -i "s/^CONSUL_HTTP_PATH=.*/CONSUL_HTTP_PATH=user_service/" "${targetDir}/.env"
